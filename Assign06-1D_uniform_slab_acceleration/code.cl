...
__kernel void molTrace(__global uchar4* pixels, float16 fcam, __global Ray* rays,
	...
	// Preparation
	float x, delta_x, tx, delta_tx, x_next, tx_next;
	...
	while(true){
		...
		for(i = slab_size[slab]; i < slab_size[slab+1]; i++){
		...
		if(champ_slab < n_slabs) break;
		t = tx_next;
		if(t >= binter.tmax) break;
		tx_next += delta_tx;
		slab += delta_xslab;
		if(slab == xslab_limit) break;
	}
	
	if(champ_slab < n_slabs){
		// Update ray's max_t
		...
		float4 fcolor = (float4)(champ_slab%3,(champ_slab+1)%3,(champ_slab+2)%3,1.0f);
		fcolor *= shade * 127.0f;
