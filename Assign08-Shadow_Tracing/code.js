...
function prepareInitTrace(){
	...
	// Create buffers.
	var acuBufferSize = width * height * 4 * 4;
	var rayBufferSize = width * height * raySize;
	var poiBufferSize = width * height * poiSize;
...
function prepareSphereTrace(){
	...
	// Set kernel arguments.
	kernels.sphereTrace.setArg(0, new Uint32Array([width, height]) );
	...
	localWS.sphereTrace = getLocalWS(dim,kernels.sphereTrace,device);
	globalWS.sphereTrace = [
		Math.ceil(width/localWS.sphereTrace[0])*localWS.sphereTrace[0],
		Math.ceil(height/localWS.sphereTrace[1])*localWS.sphereTrace[1] ];
}
...
function prepareTriangleTrace(){
	...
	// Set kernel arguments
	kernels.triangleTrace.setArg(0, new Uint32Array([width, height]) );
	...
	// Compute local and global WS
	localWS.triangleTrace = getLocalWS(dim,kernels.triangleTrace,device);
	globalWS.triangleTrace = [
		Math.ceil(width/localWS.triangleTrace[0])*localWS.triangleTrace[0],
		Math.ceil(height/localWS.triangleTrace[1])*localWS.triangleTrace[1] ];
}
...
function executeSphereTrace(){	
	...
	cmdQueue.enqueueNDRangeKernel(kernels.sphereTrace,dim,null,globalWS.sphereTrace,localWS.sphereTrace);
}

function executeTriangleTrace(){	
	cmdQueue.enqueueNDRangeKernel(kernels.triangleTrace,dim,null,globalWS.triangleTrace,localWS.triangleTrace);	
}
...
function prepareSceneRender(){
	...
	kernels.sceneRender.setArg(4,new Uint32Array([width*height]) );
	
	// Calculate local and global WS
	localWS.sceneRender = getLocalWS(1, kernels.sceneRender, device);
	globalWS.sceneRender = 
		[Math.ceil((width*height)/localWS.sceneRender[0])*localWS.sceneRender[0]];
...
function executeCopyToPixel(n_lights){
	...
	kernels.copyToPixel.setArg(2, new Float32Array([1.0/n_lights]) );
...
function prepareInitShadowTrace(){
	...
	kernels.initShadowTrace.setArg(2,new Uint32Array([width, height]) );
	//kernels.initShadowTrace.setArg(3, light.toFloat32Array() );
	
	// Calculate local and global WS
	localWS.initShadowTrace = getLocalWS(dim,kernels.initShadowTrace,device);
	globalWS.initShadowTrace = [
		Math.ceil(width/localWS.initShadowTrace[0])*localWS.initShadowTrace[0], 
		Math.ceil(height/localWS.initShadowTrace[1])*localWS.initShadowTrace[1]];
}
...
function prepareSphereShadowTrace(){
	...
	// Set kernel arguments.
	kernels.sphereShadowTrace.setArg(0, new Uint32Array([width,height]) );
	...
	// Compute local and global WS
	localWS.sphereShadowTrace = getLocalWS(dim,kernels.sphereShadowTrace,device);
	globalWS.sphereShadowTrace = [
		Math.ceil(width/localWS.sphereShadowTrace[0])*localWS.sphereShadowTrace[0],
		Math.ceil(height/localWS.sphereShadowTrace[1])*localWS.sphereShadowTrace[1] ];
}
...
function prepareTriangleShadowTrace(){
	...
	// Set kernel arguments.
	kernels.triangleShadowTrace.setArg(0, new Uint32Array([width,height]) );
	...
	kernels.triangleShadowTrace.setArg(4, bounds2AABB(scene.sphereBounds));
	kernels.triangleShadowTrace.setArg(5, new Uint32Array([n_slabs]) );
	
	// Compute local and global WS
	localWS.triangleShadowTrace = getLocalWS(dim,kernels.triangleShadowTrace,device);
	globalWS.triangleShadowTrace = [
		Math.ceil(width/localWS.triangleShadowTrace[0])*localWS.triangleShadowTrace[0],
		Math.ceil(height/localWS.triangleShadowTrace[1])*localWS.triangleShadowTrace[1] ];
}

function executeInitShadowTrace(light){
	kernels.initShadowTrace.setArg(3, light.toFloat32Array() );
	cmdQueue.enqueueNDRangeKernel(kernels.initShadowTrace,dim,null,globalWS.initShadowTrace,localWS.initShadowTrace);
}

function executeSphereShadowTrace(){
	cmdQueue.enqueueNDRangeKernel(kernels.sphereShadowTrace,dim,null,globalWS.sphereShadowTrace,localWS.sphereShadowTrace);
}

function executeTriangleShadowTrace(){
	cmdQueue.enqueueNDRangeKernel(kernels.triangleShadowTrace,dim,null,globalWS.triangleShadowTrace,localWS.triangleShadowTrace);
}
