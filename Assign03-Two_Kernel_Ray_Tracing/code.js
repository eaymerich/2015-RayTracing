...
// Prepare animation
	...
	//console.log("Ray struct size: " + raySize);
	...
	var rayBuffer = ctx.createBuffer(webcl.MEM_READ_WRITE, rayBufferSize);
	...
	var dim = 2;
	var initTraceLocalWS = getLocalWS(dim,initTraceKernel,device);
	var molTraceLocalWS = getLocalWS(dim,molTraceKernel,device);
	var initTraceGlobalWS = [Math.ceil(width/initTraceLocalWS[0])*initTraceLocalWS[0], Math.ceil(height/initTraceLocalWS[1])*initTraceLocalWS[1]];
	var molTraceGlobalWS = [Math.ceil(width/molTraceLocalWS[0])*molTraceLocalWS[0], Math.ceil(height/molTraceLocalWS[1])*molTraceLocalWS[1]];
	...
	function frame() {
		...
		initTraceKernel.setArg(1, cam.toFloat32Array() );
		molTraceKernel.setArg(1, cam.toFloat32Array() );
		cmdQueue.enqueueNDRangeKernel(initTraceKernel,dim,null,initTraceGlobalWS,initTraceLocalWS);
		cmdQueue.enqueueNDRangeKernel(molTraceKernel,dim,null,molTraceGlobalWS,molTraceLocalWS);
		...
		if(rotate === false){
			...
			atomBuffer.release();
			pixelBuffer.release();
			molTraceKernel.release();
			initTraceKernel.release();
			program.release();
			cmdQueue.release();
			ctx.release();
			clearInterval(id);
